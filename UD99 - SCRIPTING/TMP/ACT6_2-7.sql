CREATE OR REPLACE PACKAGE facturacio2 IS

	FUNCTION Obtenir_tarifa( p_tarifa VARCHAR2 ) RETURN NUMBER;

	FUNCTION Obtenir_descripcio( p_tckcod VARCHAR2 ) RETURN NUMBER;

	FUNCTION Obtenir_faccod( p_tckcod VARCHAR ) RETURN VARCHAR2;

	PROCEDURE facturar_ticket2 ( p_tckcod VARCHAR2 );

END; -- PACKAGE facturacio2


CREATE OR REPLACE PACKAGE BODY PROCEDURE facturacio2 IS

	FUNCTION Obtenir_tarifa( p_tarifa VARCHAR2 ) RETURN NUMBER IS
	...
	FUNCTION Obtenir_descripcio( p_tckcod VARCHAR2 ) RETURN NUMBER IS
	...
	FUNCTION Obtenir_faccod( p_tckcod VARCHAR ) RETURN VARCHAR2 IS
	...
	PROCEDURE facturar_ticket2 ( p_tckcod VARCHAR2 ) IS
		v_descripcio tickets2.tck_descripcio%TYPE;
		v_import	NUMBER;
		v_msg		VARCHAR2(100);
...
BEGIN
	FOR r_tck_a_facturar2 IN c_tck_a_facturar2 LOOP
		...
		facturacio2.facturar_ticket2( r_tck_a_facturar.tck_codi );
