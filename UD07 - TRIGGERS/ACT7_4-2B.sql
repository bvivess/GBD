...
CREATE OR REPLACE TRIGGER trg_employees2
	BEFORE DELETE OR UPDATE OF first_name, phone_number
	ON employees
	FOR EACH ROW
BEGIN
	IF DELETING THEN
	...
	ELSIF UPDATING THEN
		IF :OLD.first_name IS NULL  AND 
		   :NEW.first_name IS NOT NULL THEN
			DELETE FROM rmd_employees
			WHERE rmd_employee_id = :NEW.employee_id
			AND   rmd_column_name = 'FIRST_NAME';
		ELSIF :OLD.first_name IS NOT NULL  AND 
		      :NEW.first_name IS NULL THEN
			INSERT INTO rmd_employees(rmd_employee_id, rmd_column_name, rmd_text )
			VALUES(:NEW.employee_id, 'FIRST_NAME', 'First_name is null');
		END IF;
		--
		IF :OLD.phone_number IS NULL AND 
		   :NEW.phone_number IS NOT NULL THEN
			DELETE FROM rmd_employees
			WHERE rmd_employee_id = :NEW.employee_id
			AND   rmd_column_name = 'PHONE_NUMBER';
		ELSIF :OLD.phone_number IS NOT NULL  AND 
		      :NEW.phone_number IS NULL THEN
			INSERT INTO rmd_employees(rmd_employee_id, rmd_column_name, rmd_text )
